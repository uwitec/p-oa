        ...
        if (!$parameter){
            ...
            foreach ($_REQUEST as $key => $val) {
                ...
                if (!is_array($val)) {
                	...
                	foreach ($val as $k=>$v){
	                	if (in_array($v,array('gt','egt','lt','elt','eq','neq','in'))){
	                		$this->parameter.="_filter[{$k}]=".urlencode($v).'&';
	                	}elseif (is_array($v)&&count($v)==2) {
	                		$between=array_keys($v);
	                		$this->parameter.="_filter[{$k}][{$between[0]}]=".urlencode($_REQUEST[$between[0]])."&_filter[{$k}][{$between[1]}]=".urlencode($_REQUEST[$between[1]]).'&';
	                	}
                	}
                }elseif (is_array($val)) {
                	foreach ($val as $k=>$v){
                		$this->parameter.="{$val}[$k]=".urlencode($v).'&';
                	}
                }
            }
        }else {
